...
def fast_greedy_from_seperate_to_whole(a_graph):
    ...
    count = 0
    tmp_file = open('fast_greedy_community_detection_n20_m100_with_new_modularity_with_full_iter_fast_edge_drop_and_multiprocessing.txt', 'w')
    while len(left_edges) > 0:
        ...
        max_new_community_modularity, added_edge, max_modularity_community = multi_process_contro(a_graph, new_g, left_edges, adjacency_dict, degree_dict, max_new_community_modularity)
        print('added_edge', added_edge)
        print('just finished the %s th loop'%count)
...
def multi_process_contro(a_graph, a_new_g, a_left_edges, a_adjacency_dict, a_degree_dict, a_max_new_community_modularity):
    cpu_num = multiprocessing.cpu_count()
    cores = int(cpu_num * float(3/4))
    pool = multiprocessing.Pool(cores)
    pool_list = []
    for i in range(cores):
        print('this is the %s th term' %i)
        print('len(a_left_edges)', len(a_left_edges))
        if i < (cores - 1):
            pool_list.append(pool.apply_async(find_the_best_edge_to_add, args=(a_graph, a_new_g, a_left_edges[int(len(a_left_edges) * i / float(cores)): int(len(a_left_edges) * (i + 1) / float(cores))], a_adjacency_dict, a_degree_dict, a_max_new_community_modularity)))
            print('down', int(len(a_left_edges) * i / float(cores)))
            print('up', int(len(a_left_edges) * (i + 1) / float(cores)))

        else:
            pool_list.append(pool.apply_async(find_the_best_edge_to_add, args=(a_graph, a_new_g, a_left_edges[int(len(a_left_edges) * i / float(cores)):], a_adjacency_dict, a_degree_dict, a_max_new_community_modularity)))
            print('down', int(len(a_left_edges) * i / float(cores)))
    ...
    tmp_final_chosen_edge = result_list[0][1]
    tmp_final_max_modularity = result_list[0][0]
    tmp_final_community = result_list[0][2]
    for j in range(len(result_list)):
        if result_list[j][0] > tmp_final_max_modularity:
            tmp_final_chosen_edge = result_list[j][1]
            tmp_final_max_modularity = result_list[j][0]
            tmp_final_community = result_list[j][2]
    return tmp_final_max_modularity, tmp_final_chosen_edge, tmp_final_community



def find_the_best_edge_to_add(a_graph, a_new_g, a_left_edges, a_adjacency_dict, a_degree_dict, a_max_new_community_modularity):
    ss_max_modularity_community = tuple(nx.connected_components(a_new_g))
    ss_added_edge = a_left_edges[0]
    ss_max_new_community_modularity = a_max_new_community_modularity
    for edge in a_left_edges:
        tmp_community, tmp_new_modularity = new_community_and_new_modularity(a_graph, a_new_g, edge, a_adjacency_dict, a_degree_dict)
        if tmp_new_modularity > a_max_new_community_modularity:
            ss_max_new_community_modularity = tmp_new_modularity
            ss_added_edge = edge
            ss_max_modularity_community = tmp_community
    return ss_max_new_community_modularity, ss_added_edge, ss_max_modularity_community
